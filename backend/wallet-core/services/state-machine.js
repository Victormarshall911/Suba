...
// Defined state transition matrix for strict financial flow control
const TransitionMap = {
  ...
  [States.PAYMENT_RECEIVED]: [States.VALIDATING, States.FAILED, States.FLAGGED_FRAUD],
  ...
  [States.MANUAL_REVIEW]: [States.SUCCESSFUL, States.FAILED],
...
export class TransactionStateMachine {
  ...
  /**
   * Executes a status transition on a transaction and logs it.
   * Runs inside an optional database client session for transaction atomic operations.
   */
  static async transitionTo(transactionId, toStatus, changedBy = null, remarks = null, client = null) {
    ...
    try {
      ...
      // 1. Fetch transaction status under write-lock for race condition safety
      let txnResult;
      ...
      // Check if it's already in the target status (idempotent call)
